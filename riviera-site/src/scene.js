...
export async function initScene(host){
 ...
 try{
  ...
  texture=await new THREE.TextureLoader().loadAsync(`${root}riviera-images/${host.dataset.image}.webp`);
  ...
  function resize(){const r=host.getBoundingClientRect();width=r.width;height=r.height;if(!width||!height)return;renderer.setSize(width,height,false);uniforms.uAspect.value=width/height;const focus={'kite-foil-school':.8,'kitesurf-evasion':.8,'issambres-kite-school':.65,airxkite:.8};uniforms.uFocus.value=window.innerWidth<600?(focus[host.dataset.image]||.5):.5;renderer.render(scene,camera);}
  const ro=new ResizeObserver(resize);ro.observe(host);
  const io=new IntersectionObserver(entries=>{visible=entries[0].isIntersecting;if(visible)start();else{cancelAnimationFrame(frame);frame=0;}},{rootMargin:'100px'});io.observe(host);
