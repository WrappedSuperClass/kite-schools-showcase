...
if(collectionData){
 ...
 form.addEventListener('submit',e=>{e.preventDefault();const values=new FormData(form);const level=values.get('level'),sport=values.get('sport');let matches=schools.filter(s=>s.tags.includes(sport)&&s.tags.includes(level));if(!matches.length)matches=schools.filter(s=>s.tags.includes(sport));matches=matches.slice(0,3);results.replaceChildren();const description=document.createElement('p');description.textContent='A few places you might feel right at home. Explore each school, then contact the team to confirm the right session for you.';results.append(description);matches.forEach(s=>{const a=document.createElement('a');a.className='finder-result';a.href=`${root}${s.slug}/`;const im=document.createElement('img');im.src=`${root}images/${s.image}-sm.webp`;im.alt='';const content=document.createElement('div');const h=document.createElement('h3');h.textContent=s.name;const p=document.createElement('p');p.textContent=s.place+' · '+s.category;content.append(h,p);const arrow=document.createElement('span');arrow.textContent='↗';a.append(im,content,arrow);results.append(a);});const reset=document.createElement('button');reset.className='finder-reset';reset.textContent='← Change my choices';reset.addEventListener('click',()=>{results.hidden=true;form.hidden=false;form.querySelector('input').focus();});results.append(reset);form.hidden=true;results.hidden=false;results.querySelector('a').focus();});
}
const schoolData=document.querySelector('#school-data');
if(schoolData){
 const {school,courses,t}=JSON.parse(schoolData.textContent),dialog=document.querySelector('.course-dialog'),content=dialog.querySelector('.course-dialog-content');
 document.querySelectorAll('[data-course]').forEach(button=>button.addEventListener('click',()=>{const c=courses[Number(button.dataset.course)];content.replaceChildren();const eyebrow=document.createElement('p');eyebrow.className='eyebrow';eyebrow.textContent=t.courseDetail;const title=document.createElement('h2');title.textContent=c.name;title.id='course-heading';const price=document.createElement('p');price.className='dialog-price';price.textContent=c.price;const unit=document.createElement('p');unit.className='dialog-unit';unit.textContent=c.unit;const list=document.createElement('ul');c.features.forEach(f=>{const li=document.createElement('li');li.textContent=f;list.append(li);});const note=document.createElement('p');note.className='dialog-note';note.textContent=t.priceNote;const a=document.createElement('a');a.className='button button-dark';a.href=school.url;a.target='_blank';a.rel='noopener';a.textContent=t.view+' ↗';content.append(eyebrow,title,price,unit,list,note,a);openDialog(dialog);}));
